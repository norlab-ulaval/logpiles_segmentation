...
DATASET_FILENAME = 'data/prescaled/coco_annotation_rotated.json'  # COCO style annotation file
IMAGE_DIR = './data/prescaled'                                    # Folder where dataset was downloaded
SPLIT_FRAC = 0.8                                                  # Fraction of data used in train

CONFIG_FILE = "configs/config_rotated_maskrcnn.yaml"    # Detectron2 style config file
...
def init_rotated_maskrcnn(config_file, train_dicts, test_dicts, output_dir, fold_num=None, initial_weights=None): 
...
def train_rotated_maskrcnn(cfg, resume=False, show_annos=False, show_augs=False):
...
def test_rotated_maskrcnn(cfg, test_weights):
    ...
    evaluator = RotatedMaskRCNNTrainer.build_evaluator(cfg, cfg.DATASETS.TEST[0])
    val_loader = RotatedMaskRCNNTrainer.build_test_loader(cfg, cfg.DATASETS.TEST[0])
...
if __name__ == "__main__":
    ...
    cfg = init_rotated_maskrcnn(CONFIG_FILE, train_dicts, test_dicts, output_dir, initial_weights=INITIAL_WEIGHTS)

    if DO_TRAIN:
        train_rotated_maskrcnn(cfg, RESUME, SHOW_ANNOTATIONS, SHOW_AUGMENTATIONS)

    if DO_TEST:
        test_rotated_maskrcnn(cfg, TEST_WEIGHTS)
